...
module toggle_switch(anchor = BOTTOM, spin) {
  cube([13, switch_h, 9], anchor = anchor, spin = spin) {
    position(BOTTOM)
      cyl(d = 6, l = 7, anchor = TOP)
        position(BOTTOM)
          up(1)
            yrot(20)
              cyl(d1 = 3, d2 = 2, rounding1 = 1, rounding2 = 0, l = 8, anchor = TOP);
    position(TOP)
      xcopies(5, 3)
        cube([1, 2, 6], anchor = BOTTOM);
  }
}
...
box_make(print = true, explode = 30.1, hide_previews = false)
  box_shell_base_lid(box_sz, rsides = 6, wall_sides = 1.6, wall_top = 2, base_height = 3, rim_height = 2, rbot_inside = 1, rtop_inside = 1) {
    ...
    box_part(TOP + LEFT)
      box_cutout(rect(5.5, spin = 45), chamfer = 0.5);
    ...
    box_part([TOP, BOT], LEFT) {
      ...
      X(0.5)
        diff()
          cuboid(s, anchor = LEFT + BOT, rounding = -1.5, edges = BOTTOM /*,except=LEFT*/
          )
            tag("remove")
              position(TOP)
                Z(0.001) {
    ...
    box_part(LEFT)
      box_cut()
        cyl(d = laser_d, h = laser_l, anchor = BOT); // cut both base and lid

    // laser preview
    box_part(TOP + LEFT)
      box_preview("#f667")
        tag(BOX_KEEP_TAG)
          cyl(d = laser_d, h = laser_l, anchor = BOT);

    // switch
    box_part(TOP + RIGHT) {
      box_hole(7);
      box_preview("#6d67")
        toggle_switch(spin = 90);
    }
    ...
    for(a = [BACK + LEFT, FRONT + RIGHT, BACK + RIGHT, FRONT + LEFT])
      M(a * -0.5)
        position(a)
          box_screw_clamp(anchor = a, gap = 0.1);
    ...
    box_part(TOP, LEFT)
      box_preview()
        X(laser_l + 3)
          Z(1)
            cuboid(bat_sz, rounding = 2, anchor = BOTTOM + LEFT)
              position(RIGHT)
                X(1)
                  cuboid([4, 25, 15], rounding = 6, anchor = LEFT, edges = "X");

    // text
    box_part(TOP, inside = false)
      Z(-0.4)
        box_cut()
          text3d("LASER", h = 2, size = 10, atype = "ycenter", anchor = BOTTOM);


    // vents
    X(11)
      box_cut() //for(a = [BACK,FRONT])
        box_part(TOP, [BACK + LEFT, FRONT + LEFT])
          xcopies(3, 4, sp = 0)
            cuboid([1.5, 8, 8], rounding = 0.75, anchor = CENTER);
