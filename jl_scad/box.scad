...
function round_path(path, r, or, ir, closed = true) =
  let (or = get_radius(r1 = or, r = r, dflt = 0), ir = get_radius(r1 = ir, r = r, dflt = 0))
...
function lerp_index(v, x) =
  let (i = floor(x), a = v[i], b = v[min(i + 1, len(v) - 1)], f = x - i)
    lerp(a, b, f);
...
function rpath(points) =
  let (path = [
    ...
    [
      for (i = idx(path))
        each let (rr = r[i], ra = abs(rr), pt = select(path, i - 1, i + 1), angle = vector_angle(pt) / 2, prev = unit(pt[0] - pt[1]), next = unit(pt[2] - pt[1]))
          rr < 0 ? [pt[1] + prev * ra, pt[1] + next * ra] : rr > 0 ? let (d = ra / tan(angle), center = ra / sin(angle) * unit(prev + next) + pt[1], start = pt[1] + prev * d, end = pt[1] + next * d)
            (approx(angle, 90) ? [start, end] : arc(max(3, ceil((90 - angle) / 180 * segs(ra))), cp = center, points = [start, end])) : [path[i]]
    ];
...
function vector_name(v) =
  assert (is_vector(v))
    let (a = ["LEFT", "FRONT", "BOTTOM"], b = ["RIGHT", "BACK", "TOP"], l = [
      for (i = idx(v))
        if (v[i] != 0)
          v[i] < 0 ? a[i] : b[i]
    ])
...
function unless(val, a, b) =
  val == a ? b : val;

// convenience module to make an attachable without a base shape
module component(anchor = CENTER, spin = 0, orient = UP, size, size2, shift, r, r1, r2, d, d1, d2, l, h, vnf, path, region, extent = true, cp = [0, 0, 0], offset = [0, 0, 0], anchors = [], two_d = false, axis = UP, override, geom) {
  attachable(anchor, spin, orient, size, size2, shift, r, r1, r2, d, d1, d2, l, h, vnf, path, region, extent, cp, offset, anchors, two_d, axis, override, geom) {
    union() {
    } // invisible base shape
    children();
  }
}
...
module _box_shell(size, splitpoint, walls, walls_outside, halves) {
  ...
  $box_walls_xyz = [
    for (i = [0:2:5])
      walls[i] + walls[i + 1]
  ]; // left+right, back+front, top+bot
  $box_inside_ofs = [
    for (i = [0:2:5])
      walls[i] - $box_inside_overlap
  ];
  ...
  _box_layout()
    attachable($box_layout_anchor, 0, $box_layout_orient, size = sz, cp = [0, 0, sz.z / 2]) {
      if (!$box_hide_box)
        children(0);
      ...
      if (!$box_hide_parts) {
        let($box_inside = true)
          _box_inside()
            children(1); // inside box
        let($box_inside = false)
          children(1); // outside box
      }
    }
}
...
function box_half(half) =
  (is_undef(half) || is_undef($box_half)) ? true : let (half = is_list(half) && is_list(half[0]) ? half : [half])
    in_list($box_half, half);

module box_half(half, inside = true, hide = false) {
  if (box_half(half) && (is_undef(inside) || is_undef($box_inside) || inside == $box_inside) && !hide)
    children();
}
...
module box_part(half_sides, anchor = CENTER, spin, inside = true, auto_anchor = true, std_spin = false, hide = false, debug = false) {
  ...
  for(hs = half_sides) {
    matches = [
      for (half = $box_shell_halves)
        if (max(v_mul(half, hs)) == 1)
          half
    ];
    ...
    for(half = halves) {
      ...
      if (debug)
        echo(str(vector_name(hs), " -> half=", vector_name(half), " side=", vector_name(side)));
...
module box_flip(axis = RIGHT) {
  half = -$box_half;
  walls = [$box_walls[0], $box_walls[1], $box_walls[2], $box_walls[3], 
  // bot/top wall widths are swapped
  $box_walls[5], $box_walls[4], ];
  let($box_half = half, $box_walls = walls)
    rot(180, axis)
      children();
}

function box_cut_color(c) =
  default(c, default($box_cut_color, $box_inside_color));

// tag children for removal and color by $box_cut_color
module box_cut(c) tag(BOX_CUT_TAG)
  recolor(box_cut_color(c))
    children();
module box_cut_force(c) force_tag(BOX_CUT_TAG)
  color(box_cut_color(c))
    children();
...
module box_cutout(p, rounding, chamfer, depth = 0, anchor = CENTER) {
  ...
  box_cut() {
    ...
    up(0.001 + depth)
      offset_sweep(p, h, top = tprof, bottom = bprof, anchor = anchor, cp = [0, 0])
